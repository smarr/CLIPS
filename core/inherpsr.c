...
SuperclassParseError:
   DeleteClassLinks(theEnv,execStatus,clink);
   return(NULL);
  }
...
globle PACKED_CLASS_LINKS *FindPrecedenceList(
  ...
  PACKED_CLASS_LINKS *supers)
  {
   ...
   /* =====================================================================
      ...
      ===================================================================== */
   po_table = InitializePartialOrderTable(theEnv,execStatus,po_table,supers);
   ...
   if (cls != NULL)
     {
      ...
      po_table = pop;
      pop = po_table->nxt;
      RecordPartialOrders(theEnv,execStatus,po_table,cls,supers,0);
     }
   else
     pop = po_table;
   ...
   for ( ; pop != NULL ; pop = pop->nxt)
     {
      RecordPartialOrders(theEnv,execStatus,po_table,pop->cls,&pop->cls->directSuperclasses,0);
      for (i = 0 ; i < pop->cls->directSuperclasses.classCount ; i++)
        RecordPartialOrders(theEnv,execStatus,po_table,pop->cls->directSuperclasses.classArray[i],
   ...
   for (i = 0 ; i < supers->classCount ; i++)
     RecordPartialOrders(theEnv,execStatus,po_table,supers->classArray[i],supers,i+1);
         ...
         while (pop->suc != NULL)
           {
            ...
            pop->suc = stmp->nxt;
            stmp->po->pre--;
            rtn_struct(theEnv,execStatus,successor,stmp);
           }
   ...
   if (po_table != NULL)
     {
      PrintErrorID(theEnv,execStatus,"INHERPSR",5,FALSE);
      PrintClassLinks(theEnv,execStatus,WERROR,"Partial precedence list formed:",ptop);
      PrintPartialOrderLoop(theEnv,execStatus,po_table);
      while (po_table != NULL)
        {
      ...
      DeleteClassLinks(theEnv,execStatus,ptop);
      return(NULL);
     }
...
globle void PackClassLinks(
  ...
  CLASS_LINK *lptop)
  {
   ...
   for (count = 0 , lp = lptop ; lp != NULL ; lp = lp->nxt)
     count++;
   if (count > 0)
     plinks->classArray = (DEFCLASS **) gm2(theEnv,execStatus,(sizeof(DEFCLASS *) * count));
   else
     plinks->classArray = NULL;
   for (count = 0 , lp = lptop ; lp != NULL ; lp = lp->nxt , count++)
     plinks->classArray[count] = lp->cls;
   DeleteClassLinks(theEnv,execStatus,lptop);
      ...
      if (pop == NULL)
        {
         ...
         /* =============================================================
            ...
            ============================================================= */
         po_table = InitializePartialOrderTable(theEnv,execStatus,po_table,
